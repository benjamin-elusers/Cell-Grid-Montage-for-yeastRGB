...
if( ARG.length <= 3 ){ System.exit('Need 3 arguments at least'); }
...
if( File(ARG[0]).exists() ){ inputfile = ARG[0]; }else{ System.exit("TABULATED INPUT FILE DOES NOT EXIST ("+ARG[0]+")"}
if( ARG[1] ){ outdir = ARG[1]; }
if( ARG[2] !== undefined ){ screenname = ARG[2]; }
if( ARG[3] !== undefined ){ gridside = ARG[3]; }
if( ARG[4] !== undefined ){ cellsize = ARG[4]; }
if( ARG[5] !== undefined ){ picperwell = ARG[5]; }
if( ARG[6] !== undefined ){ delim = ARG[6]; }
if( ARG[7] !== undefined ){ fsep = ARG[7]; }
if( ARG[8] !== undefined ){ bitmode = ARG[8]; }
